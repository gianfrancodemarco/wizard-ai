...
class StateGraphWithGraphPng(StateGraph):
    ...
    def draw(
        ...
        for node, _ in self.nodes.items():
            graphviz_graph.add_node(node)

        # Add special node END
        graphviz_graph.add_node("END", style='filled', fillcolor='lightgreen', label="<<B>END</B>>")

        # Add special nodes for condition checking
        # Add edges to the graph
        for source, _target in self.branches.items():
            branch = _target[0]
            condition = branch.condition.__name__
            label = f"<<I>{condition}</I>>"
            graphviz_graph.add_node(condition, shape='rect', fixedsize=True, label=label, width=0.1*len(label), height=0.5)

        # Add edges to the graph
        for source, _target in self.branches.items():
            branch = _target[0]
            condition = branch.condition.__name__
            for result, target in branch.ends.items():
                if target == "__end__":
                    target = "END"
                graphviz_graph.add_edge(source, condition)
                graphviz_graph.add_edge(condition, target, label=result)

        # Set the entry point
        entry_point_node = self.entry_point
        graphviz_graph.get_node(entry_point_node).attr.update(style='filled', fillcolor='yellow')

        # Save the graph as PNG
        graphviz_graph.draw(output_file_path, format='png', prog='dot')  # 'dot' is a layout engine
        graphviz_graph.close

        # Display the graph
        Image(filename=output_file_path)
...
class MAIAssistantGraph(StateGraphWithGraphPng):
    ...
    def __init__(
        ...
        self.model = None #ChatOpenAI(temperature=0, verbose=True).bind_functions(self.functions)
