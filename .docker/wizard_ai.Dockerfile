...
COPY wizard_ai/pyproject.toml wizard_ai/poetry.lock ./
...
ENTRYPOINT ["python", "-m", "debugpy", "--listen", "0.0.0.0:5678", "-m", "uvicorn", "wizard_ai.main:app", "--reload", "--host", "0.0.0.0", "--port", "8000"]
COPY client_secret.json /

FROM base AS build_testing
ENTRYPOINT ["python", "-m", "uvicorn", "wizard_ai.main:app", "--reload", "--host", "0.0.0.0", "--port", "8000"]
